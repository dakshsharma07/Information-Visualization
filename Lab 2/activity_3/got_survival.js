...
function updateBars(house) {
    ...
    if (house == "top") {
        var filteredPower = characters.filter(checkPower);
        function checkPower(characters) {
            return characters.power_ranking >= 0;
        } 

        filteredPower.sort(function(a, b) {
        return b.probability_of_survival - a.probability_of_survival;
        });
        renderBars(filteredPower);
    } else {
    // Edit this function to filter the characters based on affiliation
    var filteredHouse = characters.filter(checkHouse);
    function checkHouse(characters) {
...
updateBars("top");
updateBars("stark");
updateBars("targaryen");
updateBars("lannister");
updateBars("baratheon");
updateBars("tyrell");
updateBars("other");
